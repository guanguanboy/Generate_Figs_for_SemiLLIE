...
# 在第一个子图上绘制数据并设置标签
line11, = axs[0].plot(width_sample_step, width_fid_visdrone, marker = 'o', markersize=10, color=custom_blue, label='Visdrone数据集(Fréchet距离)', linestyle=line_style, linewidth=2)
...
line12, = axs0_twin.plot(width_sample_step, width_psnr_LSRW, marker = '*', markersize=10, color=custom_green, label="LSRW数据集(峰值信噪比)", linestyle=line_style, linewidth=2)
axs0_twin.set_ylim([y_min, y_max])

# 在第三个子图上绘制数据并设置标签
line31, = axs[1].plot(mssg_sample_step, mssg_fid_visdrone, marker = 'o', markersize=10, color=custom_blue, label='Visdrone数据集(Fréchet距离)', linestyle=line_style, linewidth=2)

axs1_twin = axs[1].twinx()  # 注意这里使用了twinx()
line32, = axs1_twin.plot(mssg_sample_step, mssg_psnr_LSRW, marker = '*', markersize=10, color=custom_green, label="LSRW数据集(峰值信噪比)", linestyle=line_style, linewidth=2)
...
# 在第二个子图上绘制数据并设置标签
line21, = axs[2].plot(mssb_sample_step, mssb_fid_visdrone, marker = 'o', markersize=10, color=custom_blue, label='Visdrone数据集(Fréchet距离)', linestyle=line_style, linewidth=2)
